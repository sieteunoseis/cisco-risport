...
function registerCommand(program) {
  program
    .command("query")
    .description(
      "Query real-time CM device registration status (SelectCmDevice / SelectCmDeviceExt)",
    )
    .option(
      "--class <class>",
      "Any|Phone|Gateway|H323|Cti|VoiceMail|MediaResources|HuntList|SIPTrunk|Unknown",
      "Any",
    )
    .option(
      "--status <status>",
      "Any|Registered|UnRegistered|Rejected|PartiallyRegistered|Unknown",
      "Any",
    )
    .option("--model <id>", "model number or name (255 = any)", "255")
    .option(
      "--select-by <field>",
      "Name|IPV4Address|IPV6Address|DirNumber|Description|SIPStatus",
    )
    .option(
      "--item <value>",
      "single or comma-separated items (used with --select-by)",
    )
    .option("--protocol <proto>", "Any|SCCP|SIP|Unknown", "Any")
    .option(
      "--download-status <status>",
      "Any|Upgrading|Successful|Failed|Unknown",
      "Any",
    )
    ...
    .action(async (cmdOpts, cmd) => {
      ...
      try {
        ...
        let rawResults;
        if (cmdOpts.paginate) {
          const paged = await svc.selectCmDevicePaginated(
            action,
            maxReturned,
            deviceClass,
            model,
        ...
        if (devices.length === 0) {
          ...
          if (format === "table") {
            const rows = devices.map((d) => {
              const ip =
                d.IPAddress?.item?.IP ||
                d.IPAddress?.item?.[0]?.IP ||
                d.IpAddress ||
                "";
              const dirNum =
                d.LinesStatus?.item?.DirectoryNumber ||
                d.LinesStatus?.item?.[0]?.DirectoryNumber ||
                d.DirNumber ||
                "";
              return {
                Name: d.Name || "",
