...
const ALL_CLASSES = ["Phone", "Gateway", "H323", "Cti", "VoiceMail", "MediaResources", "HuntList", "SIPTrunk"];
...
function registerCommand(program) {
  program
    .command("summary")
    .description("Show registration summary across device classes (cluster health dashboard)")
    .option("--all-classes", "query all device classes (default: Phone, Gateway, SIPTrunk)")
    .option("--node <name>", "filter to a specific CUCM node")
    .option("--ext", "use SelectCmDeviceExt")
    .action(async (cmdOpts, cmd) => {
      ...
      try {
        ...
        for (const deviceClass of classes) {
          const result = await svc.selectCmDevice(action, 1000, deviceClass, "", "Any", cmdOpts.node || "", "Name", "", "Any", "Any");
        ...
        audit.log({ command: "summary", duration_ms: Date.now() - start, status: "error", error: err.message || String(err) });
        printError(err);
      }
    });
}
